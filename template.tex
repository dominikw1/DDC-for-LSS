...
applying data-driven approaches to large-scale systems, including open research challenges.
...
\tikzset{
    ...
    data/.style={rectangle, draw, dashed, minimum width=3cm, minimum height=0.8cm, align=center, fill=gray!10},
    arrow/.style={-{Stealth[length=2mm]}, thick},
    direct/.style={arrow, red!70!black},
    indirect/.style={arrow, blue!70!black}
}
...
% --- Nodes ---
\node[data] (data) {Recorded Input-Output Data};
\node[block, right=5cm of data] (K) {Controller $K$};
\node[block, right=1cm of K] (G) {Plant $G$};
\node[block, above left=1cm and 1.5cm of K] (ID) {Model $x' = f(x,u)$};

% --- Direct path ---
\draw[direct] (data.south) -- ++(0,-0.6) -| (K.south) node[pos=0.25, above] {\large Direct Synthesis};
\draw[arrow] (6,0) -- node[above] {$x$} (K);
\draw[arrow] (K) -- node[above] {$u$} (G);
\draw[arrow] (G.east) -- ++(1,0) -- ++(0,-1)node[midway, left] {$y$} -| ([xshift=0.5cm]K.south);

% --- Indirect path ---
\draw[indirect] (data.north) -- ++(0,1) |- (ID.west) node[yshift=-0.35cm, pos=0, right] {\large System Identification};
\draw[indirect] (ID.east) --  ++(1,0) -| node[yshift=-0.07cm, pos=0.8, left] {\large Model-based Synthesis} (K.north);
%\draw[indirect] (K) -- ++(0,-1.5) node[near end, left] {$u$} (G.west);
%\draw[indirect] (G.east) -- ++(2,0) node[right] {$y$} |- (K.south);

% --- Labels ---
%\node[above=0.1cm of K] {Common Controller $K$};
%\node[above=0.1cm of ID] {Indirect Path (Blue)};
%\node[below=0.1cm of K] {Direct Path (Red)};
 \matrix [
            % "draw" removed from here to hide the border
            fill=white, 
            above right=0.5cm and 1.8cm of K,
            ampersand replacement=\&,
          row sep=0.3cm,   % <--- Adjust this value for more/less space
            column sep=0.1cm %
            ] {
              \draw [red!90!black!50, dashed, thick] (0,0) -- (0.6,0);   \& \node[anchor=west] {\large Indirect DDC}; \\
              \draw [blue!70!black, dashed, thick] (0,0) -- (0.6,0); \& \node[anchor=west] {\large Direct DDC}; \\
        };

\end{tikzpicture}
\caption{Direct vs. indirect data-driven control}
\end{figure*}
...
improving this method's applicability to large-scale systems by extending the invariant property of the barrier certificate to multiple steps and integrating it into the policy optimisation in a computationally efficient manner using the Lagrangian 
method, achieving near-zero safety violations on high-dimensional control tasks.
However, the learning phase typically needs to be carried out in simulation environments, as safety constraints violations are likely at this stage.
...
\tikzset{
      block/.style={rectangle, draw, rounded corners, fill=blue!10, text centered, minimum height=0.7cm, minimum width=2cm, font=\small},
      block2/.style={rectangle, draw, rounded corners, fill=green!10, text centered, minimum height=0.7cm, minimum width=2cm, font=\small},
      block3/.style={rectangle, draw, rounded corners, fill=red!10, text centered, minimum height=0.7cm, minimum width=2cm, font=\small},
      node distance = 2.2cm,
      inner sep=6pt,
  }

  \begin{figure*}
    \label{control_archs_dig}
  % LEGEND START
  \begin{center}
    \begin{tikzpicture}[font=\small]
        \draw [->, dashed, thick] (0,0) -- (0.6,0) node[right] {\; Physical Influence};
        \draw [->, thick] (4,0) -- (4.6,0) node[right](ex) {\; Explicit Communication};
        
        \node[block, scale=0.6, right of=ex, xshift=2cm] (sub) {Subsystem};
        \node[block2, scale=0.6, right of=sub, node distance=2.8cm] (mid) {Level 2 Controller};
        \node[block3, scale=0.6, right of=mid, node distance=2.8cm, xshift= 0.3cm] (high) {Level 1 Controller};
        
        % Descriptive text for nodes
%        \node[below of=mid, node distance=0.6cm, font=\itshape\footnotesize] {Nodes represent various control entities};

      \end{tikzpicture}
  \end{center}
  \vspace{0cm}
  % LEGEND END
  % Column 1: Decentralised
  \begin{minipage}[t]{0.32\linewidth}
      \centering
      \begin{tikzpicture}[scale=0.5, transform shape]
          \node[block](A) {A};
          \node[block, right of=A, node distance=3.5cm](B) {B};
          \node[block, below of=B](D) {D};
          \node[block, below of=A](C) {C};
          \draw [->, dashed, looseness=0.5] (A) to [bend right=60] (C);
          \draw [->, dashed, looseness=0.5] (A) to [bend right=20] (D);
          \draw [->, dashed, looseness=0.5] (D) to [bend right=20] (B);
          \draw [->, dashed, looseness=0.5] (C) to [bend right=20] (D);
      \end{tikzpicture}
      
      \vspace{0.2cm}
      \textbf{Decentralised Control}\\
      \small Subsystems influence each other, but no explicit communication is taking place.
  \end{minipage}\hfill
  % Column 2: Distributed
  \begin{minipage}[t]{0.32\linewidth}
      \centering
      \begin{tikzpicture}[scale=0.5, transform shape]
          \node[block](A) {A};
          ...
          \draw [->, dashed, looseness=0.5] (A) to [bend right=60] (C);
          \draw [->, dashed, looseness=0.5] (A) to [bend right=20] (D);
          \draw [->, dashed, looseness=0.5] (D) to [bend right=60] (B);
          \draw [->, dashed, looseness=0.5] (C) to [bend right=20] (D);
          %\draw [->] (A) -- (B);
          %\draw [->] (A) -- (D);
          ...
          \draw [<->] (A.east) -- (B.west);
          \draw [<->] (C.east) -- (D.west);
          \draw [<->] ([xshift=0.2cm]C.north) -- ([xshift=-0.3cm]B.south);
          \draw [<->] ([xshift=0.2cm]A.south) -- ([xshift=-0.3cm]D.north);
      \end{tikzpicture}

      \vspace{0.2cm}
      \textbf{Distributed Control}\\
      \small Subsystems influence and can communicate with each other, though potentially with delays.
  \end{minipage}\hfill
  % Column 3: Hierarchical
  \begin{minipage}[t]{0.32\linewidth}
      \centering
      \begin{tikzpicture}[scale=0.5, transform shape]
          \node[block](A) {A};
          ...
          \node[block, below of=B](D) {D};
          \node[block, below of=A](C) {C};
          \node[block2, below of=A, node distance=1.1cm, xshift=-2cm](alpha) {$\alpha$};
          \node[block2, below of=B, node distance=1.1cm, xshift=2cm](beta) {$\beta$};
          \node[block3, above of=A, xshift=1.75cm, yshift=-0.7cm](xi) {$\xi$};
          
          \draw [<->, dashed, looseness=0.5] (A) to [bend right=60] (C);
          \draw [<->, dashed, looseness=0.5] (A) to [bend right=20] (D);
          \draw [<->, dashed, looseness=0.5] (D) to [bend right=20] (B);
          \draw [<->, dashed, looseness=0.5] (C) to [bend right=20] (D);
          \draw [<->] (A.west) to [bend right=20] (alpha.north);
          \draw [<->] (C.west) to [bend left=20] (alpha.south);
          \draw [<->] (B.east) to [bend left=20] (beta.north);
          \draw [<->] (D.east) to [bend right=20] (beta.south);
          \draw [<->, looseness=0.5] (alpha) to [bend left=50] (xi.west);
          \draw [<->, looseness=0.5] (beta) to [bend right=50] (xi.east);
      \end{tikzpicture}

      \vspace{0.2cm}
      \textbf{Hierarchical Control}\\
      \small Communication happens primarily with entities directly above and below them in the hierarchy.
  \end{minipage}
   \caption{An overview of non-centralised control architectures}
\end{figure*}
...
In this survey, in line with existing literature~\cite{kordestani2021recent}, decentralised control is understood to not allow for explicit communication between subsystems, whereas distributed control does, as illustrated in \cref{control_archs_dig}.
...
despite the limited information exchange between subsystems.
...
Overall, decentralised robustness mechanisms scale well computationally with respect to number of subsystems, 
...
As opposed to decentralised control, subsystems in distributed architectures explicitly share information among system components.
...
improve flexibility, they introduce new challenges related to computational complexity, stability guarantees and safety certificates.
...
Future investigation could focus on novel robustness and safety approaches that leverage the overall system topology,
instead of relying on the common practice of decomposing global objectives into tendentially conservative local constraints.
