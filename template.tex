...
Model-based control tends to be insufficient for complex systems, though~\cite{DDC_Overview}.
...
whereas a model-based technique requries expert-knowledge for deriving the model from first principles,
the specific result then being domain-specific. SI further provides the applicant with 
...
We try to make it smaller using strategies in Model Order Reduction (MOR) methods.
This technique attempts to generate
reduced models reproducing the input-output behavior 
of large-scale systems accurately.
Model reduction methods two categories: SVD-based and Krylov-based or moment matching methods~\cite{lowenerIntro}.


\subsection{Control Architectures}

We have funny strategies such as decentralised and distributed control.

Decentralised does not actively share date, distributed does. There tend to be delays though.
...
When a linear model is too large to identify directly, 
one can use data-driven model reduction. 

The Loewner framework constructs a reduced linear
model from transfer-function data or sampled impulse response~\cite{gosea2021datadrivenmodelingcontrollargescale}.
...
A similar method is presented in \cite{gosea2021datadrivenbalancinglineardynamical},  a novel reformulation of balanced truncation (BT).
BT~\cite{balancedtruncationOriginal} is a widely successfully employed MOR technique for large systems~\cite{salehi2021mixed}. 
