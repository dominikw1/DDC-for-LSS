...
dynamics, render classical centralised control approaches difficult to model,
...
This survey aims to provide a structured overview of data-driven control for large-scale systems, emphasising how modeling choices, control paradigms and architectural design jointly determine scalability. 
It highlights trade-offs between indirect and direct data-driven control, 
centralised and non-centralised architectures, and modeling fidelity versus computational tractability.
...
Such model-based control can become intractable or inaccurate for complex large-scale systems, though~\cite{DDC_Overview}.
...
insight into the mechanisms driving system behaviour, 
black-box controllers derived purely from data can be 
difficult to analyse or verify~\cite{Brunton_Kutz_2022}. 
Finally, issues of robustness and generalisation remain 
...
The term is rather used whenever a system grows unwieldy to control with traditional strategies.
...
the decomposition of the global control problem into the control of subsystems
...
an approach more suited for large-scale systems, as obtaining partitions by physical reasoning can become intractable~\cite{ocampo2011partitioning}.
...
Koopman lifting remains viable only for moderate-size subsystems with coherent low-dimensional
...
Kernelized DeePC~\cite{huang2023robust} enables the efficient control of nonlinear systems.
...
seeking a function \(f\in \mathcal{H}\) minimizing a regularised least-squares objective of the form

\begin{equation}
f^* = \argmin_{f\in\mathcal{H}}\sum_{j=1}^N ||y_j-f(z_j)||^2 + \gamma||f||_\mathcal{H}^2
\end{equation}

where \(z_j\) is a suitably defined past-input/output regressor, \(\gamma\) a regularisation parameter
...
Given this predictor, control is then formulated as a robust min-max optimisation over disturbance sets on inputs and outputs.
To efficiently solve this problem generally non-convex in \(u\), customised gradient descent methods tailored to the RoKDeePC structure are proposed.
...
the former providing data-driven reachability sets 
...
To address these challenges, the three principal paradigms of decentralised, distributed, and hierarchical control have evolved, each with characteristic trade-offs in scalability, performance, and robustness. 
...
the goal is to minimise some some finite-horizon criterion \(J(\mathbf{u}_t, \mathbf{y}_t)\). With reference trajectory \((\bar{\mathbf{y}}_t, \bar{\mathbf{u}}_t)\), this could take the form of the quadratic criterion
...
A high-level policy learns joint cooperative behaviour, while low-level controllers enforce individual safety via control barrier functions.
