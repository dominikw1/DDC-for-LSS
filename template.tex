...
NOTE: The background section is much more in a draft state than the other sections.
...
Model Order Reduction (MOR) methods tackle the problem of reducing the complexity of large-scale systems.
...
be categorised into decentralised, distributed and hierarchical control architectures, in addition to hybrids thereof.[cite]
...
such that safety constrants are respected and the modification is minimal, i.e. minimising \(\int_{0}^\infty||\kappa(x(t),u_{contr}(t))-u_{contr}(t)dt\).
Among these, three major methodological families 
dominate current research: Control Barrier Function (CBF) 
approaches, Hamilton-Jacobi (HJ) reachability-based methods, and Predictive control 
...
Hamilton-Jacobi (HJ) reachability analysis computes the backward reachable tube
(BRT), a collection of all initial states with trajectories eventually reaching a target set even under the worst-case disturbance.
For safety, this target set can be the set of all unsafe states, the BRT then representing all potentially unsafe, to-be-avoided states~\cite{borquez2024safety}.
...
Predictive safety filters (PSFs) 
...
Whereas system identification seeks compact dynamic representations,
topology identification extends this goal to structural discovery.
...
\subsection{Data-enabled Predictive Control (DeePC)}

Data-enabled Predictive Control (DeePC)~\cite{coulson2019data} is 
...
Taking as input a reference trajector \(r\), past input-output data \((u_{ini}, y_{ini})\), constraint sets \(U,Y\) and performance matrices \(Q\) amd \(R\),
...
\cite{wang2024mechanical} adds a dimension reduction step to the traditional DeePC 
technique, greatly improving scalability.
...
Kernelized DeePC (RoKDeePC) enables the control of nonlinear systems,
reformulating the optimisation as a nonconvex but
structured minimization problem.~\cite{huang2023robust}. 
The core idea is to take the advantages of both robust predictive control and 
regularised kernel methods, namely the non-parametric representation of nonlinear systems.
Using a projected gradient descent algorithm, the optimal control action can be determined efficiently.
...
Jong et al. propose using a product reproducing-kernel Hilbert space (RKHS) 
...
form via Singular Value Decomposition (SVD). Then a fast SVD update mechanism is provided
...
Robust DeePC variants employ
distributional robustness concepts,
e.g., Wasserstein ambiguity sets or
chance-constrained formulations,
to ensure performance under measurement
noise and modeling uncertainty~\cite{coulson2019regularized}.
\cite{huang2023robust} proposes a computationally tractable reformulation of DeePC's traditional min-max optimisation
problem via uncertainty sets, providing performance guarantees for the open-loop realized input/output cost.
For partially known systems, hybrid data-enabled predictive control (HDeePC)~\cite{watson2025hybrid}
...
Unlike classical MPC where a
model provides invariant sets and Lyapunov proofs, 
DeePC lacks an explicit parametric model and 
thus must embed safety via data-driven mechanisms.
...
Generally, robustness approaches in Reinforcement Learning can be categorised into four classes, as proposed by Moos et al.~\cite{moos2022robust}:
Transition robust methods adjust the transition probabilities between states,
disturbance robust ones model uncertainty via external forces, 
action robust techniques corrupt agents' outputs in order to redirect system transitions,
and observation robust methods change the policy's perceived system state.
In the transition uncertainty case, the maximisation problem presented in 
\cref{eq:reinforcement} might be reformulated as follows~\cite{moos2022robust}, assuming an unknown transition matrix \(P\) to be fixed and lying within a known uncertainty set \(\mathcal{U}_P\):
...
\subsubsection{Decentralised Model Predictive Control}
...
Chen et al.~\cite{chen2020decentralized} proposes a decentralised MPC system for nonlinear processes
...
A central approach for robust decentralised predictive control 
introduces bounded uncertainty
sets for each subsystem. In distributed 
contexts these arise not only from local
disturbances or measurement noise, but
also from inter-subsystem coupling or network effects
whose characteristics are only partially observed.
...
Outside of the realm of predictive control, \cite{li2023decentralized} develops a robust decentralized data-driven control 
scheme for large-scale interconnected systems based on adaptive dynamic programming 
subject to actuator faults, external disturbances,
and model uncertainties. 
The key idea is to cast the decentralized control problem
into a zero-sum differential game framework,
where each subsystem minimizes a performance index while counteracting the worst-case disturbance. 
To achieve this without requiring knowledge of
system dynamics, the solution of the Hamilton-Jacobi-Isaacs equation is iteratively approximated using only input-output data. 
Each subsystem locally learns its optimal control
and disturbance strategies through a two-stage ADP
algorithm based on policy or value iteration, ensuring
scalability and robustness.
Furthermore, adaptive parameters are introduced to
dynamically compensate for unknown and time-varying
actuator faults.
The resulting decentralized controllers are proven to 
asymptotically stabilize the overall system and guarantee a
bounded gain from disturbance to output,
as demonstrated in simulations on a multimachine power system.
...
despite only implicitly gaining information from the other subcontrollers, no direct communication channel
being available, safety hence having to be enforced locally and independently.


\cite{zhang2023data} proposes a decentralised deep reinforcement learning approach for Inverter-Based Renewable Energy Sources.
Its safety is guaranteed via an embedded safe policy projection mechanism, a kind of safety filter.
This approach operates by decoupling the learning process from constraint 
adherence, 
integrating a Guaranteed Safety Controller (GSC) 
directly into the action selection loop of each local deep reinforcement learning agent (DRL).
The GSC uses real-time local state measurements and
analytical system knowledge to dynamically calculate a
Safety-Critical Action Set (SCAS), which is the
provably non-violating range of control inputs for that specific instant.
The DRL policy proposes an unconstrained, 
optimal action \(a_{\text{DRL}}\), the final executed 
action \(a_{\text{final}}\) is 
determined by projecting \(a_{\text{DRL}}\) onto the SCAS, \(a_{\text{final}} = \text{Projection}(a_{\text{DRL}}, \text{SCAS})\). 
This guarantee ensures that regardless of the exploratory or off-policy nature of the DRL action, the physical control command always adheres to local constraints, resulting in a system with $100\%$ constraint satisfaction during operation.
Crucially, the success of this
policy projection method hinges on the existence of a
tractable local analytical model that permits the real-time
calculation of the SCAS. 
This represents a hybrid approach where the 
long-term optimality is data-driven, but the hard safety
boundaries are model-derived and known a priori.

As an alternative approach, the model-dependance can be
removed via integrating a Graph Neural Network (GNN), as shown in ~\cite{hashemnezhad5371728safe} for the purpose of fast, local power flow estimation.
This GNN is trained on historical data to serve as a data-driven safety predictor, quickly estimating the effect of the agent's proposed action on local state variables.
The estimated state is then checked against the operational constraints. 
If a violation is predicted, a separate 
DNN-based action correction mechanism modifies the agent's 
action to the nearest safe point.

Comparatively little research is being conducted on developing safe and robust data driven controllers for 
decentralised architectures. One reason for this could be the relatively disadvantageous starting position,
no communication being possible between subcontrollers, whereas global safety constraints must be met at all times.
However, not all systems allow for communication, making the design of such controllers
an important task despite its difficulty.
...
direction method of multipliers (ADMM)~\cite{rostami2017admm}, the dual fast gradient method~\cite{wu2024iterative}
...
use the System Level Synthesis (SLS) framework~\cite{anderson2019system} to generate a distributed model-predictive controller, which shares information only locally, the neighbourhood sizes being adjustable.
The SLS framework parameterizes the entire closed-loop system using two transfer matrices called system responses \(\{\Phi_x, \Phi_u\}\), such that 
...
\cite{bu2025event} developed a data-driven control algorithm
...
\cite{10337762} proposes a method for connected heterogeneous vehicle platoon control
considering sensor faults, network resource usage minimised through the effective use of triggering conditions.
\cite{yu2023distributed} develops an iterative learning approach for the control of 
Multiple High-Speed Trains (MHSTs) with sitching topologies, proving 
...
A class of distributed robustness strategies tackles
network-scale uncertainty by combining data-driven uncertainty
quantification with distributed optimization and local certified control.
In \cite{wang2024data} each subsystem of the interconnected
network uses measurement data to infer uncertainty sets for both its local dynamics and couplings
and then participates in a distributed robust control optimisation
that ensures global constraint satisfaction despite
the worst-case admissible uncertainties.
The robustness is achieved through three key ideas: 
the data-driven construction of uncertainty bounds from historical measurements, 
the distributed robust optimization so that each node enforces tightened coupling and constraint bounds under communication and coordination constraints, respectively, and 
the fact that local controllers execute the optimized signals while guaranteeing invariant-set membership or disturbance rejection within the prescribed uncertainty set.
...
and iteratively updates its control strategy via a distributed ADMM
algorithm enhanced with Nesterov acceleration, enabling scalability. 
...
the distributed optimization architecture where only limited peer exchange is required and convergence is accelerated, and
decentralized local correction at each node which ensures the system stays robust to both local disturbance and coupling uncertainties.
...
\cite{khaledi2025data} constrains a direct data-driven distributed controller for multi agent systems
via Control Barrier Functions incorporating quadratic programming optimisations. 
The online computational complexity is dominated by the interior-point QP, which is, for \(m_i\) being the control input
dimension for agent \(i\), \(\mathcal{O}((\sum_im_i)^3)\) due to the factorization of
the Karush-Kuhn-Tucker (KKT) system for dense data~\cite{ye1989extension}, making it suitable for suitable for real-time computation in moderately sized systems.

\cite{convens2022safe} formulates a novel Trajectory-Tube
Distributed Explicit Reference Governor (TT-D-ERG) approach for the control of aerial robot swarms.
The core idea is to predict the future higher-order nonlinear dynamics and
embed it in a time-varying safety tube, information about which is shared with the neighbours.
They identified their technique to achieve provably safe constrained control of resource constrained
UAV swarms, given its low requirements of computational resources and has little communication overhead, 
scaling well for even large swarms.
...
Hierarchical Model Predictive Control (HMPC) extends MPC to a multi-level framework, where upper layers solve relaxed or aggregated optimization problems whose 
outputs guide lower layers~\cite{SCATTOLINI2009723}. 

\cite{shi2020data} presents an indirect data-driven hierarchical control structure for systems operating under uncertainty.
...
One of the early papers in the field of hierarchical reinforcement learning (HRL) was \cite{singh1992transfer},
...
laying the groundwork for the widely successful HRL framework. 

% defintiion for HRL https://arxiv.org/pdf/2011.04752

In \cite{chen2023distributed}, a distributed hierarchical controller using deep reinforcement learning is proposed.
They develop a new framework coined hierarchical reduction reinforcement learning (HR2L),
improving upon prior computational efficiency issues, generally impriving upon the prior art in regards to
convergence speed, solution quality, training robustness, and adaptability to large-scale systems.
