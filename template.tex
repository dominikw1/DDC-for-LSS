...
Shang et al.~\cite{shang2024decentralized} propose a 
decentralized robust data-driven predictive control scheme in which each
subsystem estimates time-varying disturbance bounds from local data and
incorporates them into its predictive optimization, yielding stable and constraint-satisfying
closed-loop behavior even under significant coupling uncertainty.
Comparable ideas have also been successfully applied in other domains, 
such as microgrids, where local controllers infer disturbance bounds from voltage and
power measurements using voltage and power measurements to maintain
stability under communication and measurement noise~\cite{s22228709}.
Beyond predictive control, Li et al.~\cite{li2023decentralized} develop a robust decentralized
data-driven control framework based on adaptive dynamic programming.
The decentralized problem is formulated as a zero-sum differential game, with each subsystem counteracting worst-case disturbances,
symptotically stabilising the system while guaranteeing a bounded disturbance-to-output gain.

Overall, decentralized robustness mechanisms scale well computationally, usually linearly with the number of subsystems, as they avoid global optimization and communication entirely.
However, this scalability does not stem from exploiting the large-scale structure of the system,
...
Zhang et al.~\cite{zhang2023data} propose a decentralised deep reinforcement learning approach 
for Inverter-Based Renewable Energy Sources.
Its safety is guaranteed via an embedded safe policy
projection mechanism, operating by decoupling the learning process from constraint 
adherence, integrating a Guaranteed Safety Controller
directly into the action selection loop of each local deep reinforcement learning agent.
The action proposed by the deep reinforcement learning policy is projected onto a safety-critical action set 
before execution, ensuring constraint satisfaction regardless of exploration.
This hybrid architecture guarantees local safety but relies
on the availability of a tractable local model for the computation of the action set.

As an alternative approach, the model-dependance can be
removed via integrating a graph neural network, 
as shown in ~\cite{hashemnezhad5371728safe} for the purpose of fast, local power flow estimation.
A graph neural network is trained on historical data to estimate
the local impact of control actions on system states, enabling fast safety checks. 
If a constraint violation is predicted,
a separate neural network modifies the action to the nearest safe input. 
While fully data-driven, this approach again enforces safety locally and does
not provide compositional guarantees for global constraints.

Overall, research on safety for decentralized data-driven control remains limited.
