...
with approximately constant online complexity per subsystem, as they avoid global optimization and communication entirely.
...
where it preserves computational scalability by confining expensive optimization and adaptation to
...
A high-level policy learns joint cooperative behavior, while low-level controllers enforce individual safety via control barrier functions.
Safety-critical quadratic programs are solved only at the agent level and depend solely on local state
and interaction neighborhoods, keeping per-agent
...
These primarily include the exploitation of large-scale structure for robust and safety controllers in networked systems.
Research in all presented control architectures remains limited and commonly tends to employ standard safety
