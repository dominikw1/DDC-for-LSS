...
Section II provides a brief background on data-driven control, large-scale systems and safety filters, the central
technique for providing formal guarantees for data-driven controllers. 
Section III reviews data-driven modeling techniques relevant to large-scale systems, enabling system decomposition and dimensionality reduction. 
Section IV covers direct, model-free control approaches and their extensions to large-scale and distributed settings. 
Section V discusses decentralized, distributed, and hierarchical control architectures, analyzing how architectural choices affect scalability, 
...
Section VI concludes with a brief discussion of open challenges and directions for future research.
...
Zhang et al.~\cite{zhang2023data} propose a decentralised deep reinforcement learning approach for Inverter-Based Renewable Energy Sources.
...
adherence, 
integrating a Guaranteed Safety Controller
directly into the action selection loop of each local deep reinforcement learning agent (DRL).
The Guaranteed Safety Controller uses real-time local state measurements and
...
removed via integrating a graph neural network, as shown in ~\cite{hashemnezhad5371728safe} for the purpose of fast, local power flow estimation.
This graph neural network is trained on historical data to serve as a data-driven safety predictor, quickly estimating the effect of the agent's proposed action on local state variables.
...
Reaseach on developing safe and robust data-driven controllers for 
decentralised architectures remains comparatively sparse. 
One reason for this could be the relatively disadvantageous starting position,
...
direction method of multipliers algorithm enhanced with Nesterov acceleration, enabling scalability. 
...
the Karush-Kuhn-Tucker system for dense data~\cite{ye1989extension}, making it suitable for suitable for real-time computation in moderately sized systems.

Alternatively, a novel Trajectory-Tube
Distributed Explicit Reference Governor approach can be formulated, as shown on the example of control of aerial robot swarms~\cite{convens2022safe}.
